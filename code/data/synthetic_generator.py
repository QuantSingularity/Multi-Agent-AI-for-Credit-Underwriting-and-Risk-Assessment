...
class SyntheticDataGenerator:
    ...
    def generate_applications(self, n_samples: int = 1000, 
                             default_rate: float = 0.20) -> pd.DataFrame:
        """
        ...
        race = self.rng.choice(['White', 'Black', 'Hispanic', 'Asian', 'Other'], 
        ...
        # Home ownership
        home_ownership = self.rng.choice(['RENT', 'OWN', 'MORTGAGE', 'OTHER'],
                                        n_samples, p=[0.35, 0.12, 0.50, 0.03])
        
        # Debt-to-income ratio
        monthly_debt = revolving_balance * 0.03 + loan_amount * 0.02  # Approx monthly payments
        ...
        logger.info(f"Generated {n_samples} applications with {loan_status.sum()} defaults ({loan_status.mean():.2%})")
        
        return df
    
    def _calculate_default_probability(self, credit_utilization, delinquencies, 
                                      debt_to_income, inquiries, loan_to_income) -> np.ndarray:
        """
        ...
        default_prob = 1 / (1 + np.exp(-risk_score + 2))  # Shift to get reasonable range
