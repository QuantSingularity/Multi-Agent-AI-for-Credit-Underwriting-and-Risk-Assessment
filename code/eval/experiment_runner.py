...
class ExperimentRunner:
    ...
    def _prepare_data(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, np.ndarray, pd.DataFrame]:
        """
        ...
        feature_cols = [
            ...
            'debt_to_income_ratio', 'credit_lines_open', 'total_credit_limit',
            'revolving_balance', 'credit_utilization', 'delinquencies_2y',
            'inquiries_6m', 'oldest_account_months'
        ]
        
        # One-hot encode categorical features
        df_encoded = df.copy()
        df_encoded = pd.get_dummies(df_encoded, columns=['home_ownership', 'loan_purpose'], 
                                    drop_first=True)
        
        # Get feature columns (including one-hot encoded)
        available_features = [col for col in df_encoded.columns 
                            if col in feature_cols or col.startswith(('home_ownership_', 'loan_purpose_'))]
    ...
    def _evaluate_baselines(self, X_train, y_train, X_test, y_test, 
                           sens_test) -> Dict[str, Any]:
        ...
        baselines["decision_tree"] = self._compute_metrics(y_test, y_pred_dt, "DecisionTree")
        
        # 4. LightGBM
        logger.info("Training LightGBM...")
        lgbm = lgb.LGBMClassifier(random_state=self.random_seed, n_estimators=100,
                                 learning_rate=0.05, max_depth=6, verbose=-1)
        ...
        for name, metrics in baselines.items():
            logger.info(f"  {name}: AUC={metrics['auc']:.4f}, AP={metrics['avg_precision']:.4f}")
        
        return baselines
    
    def _evaluate_agentic_system(self, X_train, y_train, X_test, y_test,
                                sens_test, df_full) -> Dict[str, Any]:
        ...
        agentic_results["lightgbm"] = self._compute_metrics(y_test, y_pred_agent, "Agentic-LightGBM")
        
        logger.info(f"\nAgentic System: AUC={agentic_results['lightgbm']['auc']:.4f}")
        
        return agentic_results
    
    def _evaluate_fairness(self, X_train, y_train, sens_train, 
                          X_test, y_test, sens_test) -> Dict[str, Any]:
        ...
        model_baseline = lgb.LGBMClassifier(random_state=self.random_seed, 
        ...
        fairness_results["baseline"] = {
            ...
            "model_performance": self._compute_metrics(y_test, y_pred_baseline, "Baseline")
        }
        ...
        model_reweighed = lgb.LGBMClassifier(random_state=self.random_seed,
                                            n_estimators=100, verbose=-1)
        ...
        fairness_results["reweighing"] = {
            ...
            "model_performance": self._compute_metrics(y_test, y_pred_reweighed, "Reweighed")
        }
        ...
        fairness_results["threshold_opt"] = {
            ...
            "model_performance": self._compute_metrics(y_test, y_pred_optimized, "ThresholdOpt")
        }
        
        logger.info("\nFairness Evaluation Summary:")
        for name, result in fairness_results.items():
            logger.info(f"  {name}: Passed={result['passed']}, AUC={result['model_performance']['auc']:.4f}")
        
        return fairness_results
    
    def _run_ablations(self, X_train, y_train, X_test, y_test, sens_test) -> Dict[str, Any]:
        ...
        for model_name, model in [
            ("logistic", LogisticRegression(random_state=self.random_seed, max_iter=1000)),
            ("lightgbm", lgb.LGBMClassifier(random_state=self.random_seed, n_estimators=100, verbose=-1)),
            ("neural_net", MLPClassifier(hidden_layer_sizes=(64, 32), random_state=self.random_seed, max_iter=300))
        ]:
            model.fit(X_train, y_train)
            y_pred = model.predict_proba(X_test)[:, 1]
            ablations[f"model_{model_name}"] = self._compute_metrics(y_test, y_pred, model_name)
    ...
    def _run_statistical_tests(self, results: Dict) -> Dict[str, Any]:
        ...
        if "lightgbm" in results["baselines"] and "lightgbm" in results["agentic_system"]:
